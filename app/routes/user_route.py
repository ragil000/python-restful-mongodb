...
def read_all():
        try:
            data = user_model.UserModel.objects()
...
def create():
        try:
            ...
            data = user_model.UserModel(**body).save()
...
def sign_in():
        try:
            ...
            data = user_model.UserModel.objects(username=body['username']).first()
            check_password = data.check_password(body['password'])
            print('ini hasil cecknya', check_password)
            response = ok_response(data=data)
            return response
        except:
            print("Unexpected error:", sys.exc_info())
